...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'eventDate' | 'eventEndDate'> & {
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...
      eventEndDate: event.eventEndDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...
      eventEndDate: restEvent.eventEndDate ? dayjs(restEvent.eventEndDate) : undefined,
    };
  }
